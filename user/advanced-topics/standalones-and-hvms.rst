A `standalone </user/reference/glossary.html#standalone>`__ is a type of qube that is
created by cloning a `template </user/reference/glossary.html#template>`__. Unlike
...
(HVM) </user/reference/glossary.html#hvm>`__, also known as a “Fully-Virtualized
...
must be updated as an independent qube. See :doc:`How to
...
Qubes are template-based (i.e., `app qubes </user/reference/glossary.html#app-qube>`__
...
Qubes (through `keys </developer/debugging/vm-interface.html#qubesdb>`__ exposed by dom0 to
...
qube </user/reference/glossary.html#net-qube>`__.) In this instance, our DHCP server is
not useful.
...
`opt-in support </developer/system/networking.html#ipv6>`__ for IPv6 forwarding.
...
HVMs (including Windows qubes) can be :doc:`assigned PCI
devices </user/how-to-guides/how-to-use-pci-devices>` just like normal app qubes.
