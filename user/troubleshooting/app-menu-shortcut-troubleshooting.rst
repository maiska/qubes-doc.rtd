...
2. Create a custom ``.desktop`` file in ``~/.local/share/applications``.
   ...
   scenes </user/troubleshooting/app-menu-shortcut-troubleshooting.html#behind-the-scenes>`__
   for more details.
...
``qvm-sync-appmenus`` works by invoking *GetAppMenus* :doc:`Qubes
service </developer/services/qrexec>` in the target domain. This service enumerates
