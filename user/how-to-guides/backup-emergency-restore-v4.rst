...
the program from source yourself. (Don’t forget to :doc:`verify
signatures </project-security/verifying-signatures>` first!) Note that
...
1. If you’re not on Qubes 4.X, `import and authenticate the Release 4
   Signing
   Key </project-security/verifying-signatures.html#how-to-import-and-authenticate-release-signing-keys>`__.
...
3. Verify the integrity of ``backup-header``. For compatibility reasons,
   ...
   **Note:** If this command fails, it may be that the backup was
   ...
   -  :doc:`Emergency Backup Recovery without Qubes
      (v2) </user/how-to-guides/backup-emergency-restore-v2>`
   -  :doc:`Emergency Backup Recovery without Qubes
      (v3) </user/how-to-guides/backup-emergency-restore-v3>`
