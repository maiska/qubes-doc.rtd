...
Selecting “Yes to All” will add a line in the corresponding :doc:`RPC
Policy </user/advanced-topics/rpc-policy>` file.
...
-  ``vault``
   ...
   `copied </user/how-to-guides/how-to-copy-and-move-files.html#security>`__ or
   `pasted </user/how-to-guides/how-to-copy-and-paste-text.html#security>`__ into this VM
...
-  ``work-gpg``
   ...
   secret key) are
   `copied </user/how-to-guides/how-to-copy-and-move-files.html#security>`__ from the
   ``vault`` VM to this VM. Files from less trusted VMs are *never*
   `copied </user/how-to-guides/how-to-copy-and-move-files.html#security>`__ into this VM
   under *any* circumstances.
...
default) via the Split GPG protocol or other :doc:`covert
channels </user/security-in-qubes/data-leaks>`. Once the master secret key is in the
...
template </user/templates/templates.html#trusting-your-templates>`__ upon which the
``vault`` VM is based.
