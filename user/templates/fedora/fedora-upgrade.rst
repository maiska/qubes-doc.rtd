...
:doc:`Fedora Template </user/templates/fedora/fedora>` page instead. (`Learn more
about the two options. </user/templates/fedora/fedora.html#upgrading>`__)
...
the new template. </user/templates/templates.html#switching>`__
...
3.  Attempt the upgrade process in the new template.
    ...
    **Note:** If you encounter no errors, proceed to step 4. If you do
    ...
    -  ``dnf`` may complain:
       ...
       image </user/advanced-topics/resize-disk-image>` before reattempting the
...
5.  (Optional) Trim the new template. (This should `no longer be
    necessary </user/templates/templates.html#important-notes>`__, but it does not
...
10. (Optional) `Uninstall the old
    template. </user/templates/templates.html#uninstalling>`__ Make sure that the
...
1. :doc:`Increase the template’s disk image
   size </user/advanced-topics/resize-disk-image>`. This is the solution mentioned in
...
4. Do not perform an in-place upgrade, see `Upgrading Fedora
   templates </user/templates/fedora/fedora.html#upgrading>`__.
