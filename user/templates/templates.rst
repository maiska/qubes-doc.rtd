...
`templates </user/reference/glossary.html#template>`__. Each template shares its root
filesystem (i.e., all of its programs and system files) with all the
qubes based on it. `App qubes </user/reference/glossary.html#app-qube>`__ are where you
...
-  `Fedora Minimal </user/templates/minimal-templates>`
-  :doc:`Fedora Xfce </user/templates/xfce-templates>`
-  :doc:`Debian </user/templates/debian/debian>`
-  `Debian Minimal </user/templates/minimal-templates>`
...
-  :doc:`Whonix </external/privacy-guides/whonix>`
-  :doc:`Ubuntu </external/os-guides/ubuntu>`
-  :doc:`Arch Linux </external/building-guides/building-archlinux-template>`
-  :doc:`CentOS </external/os-guides/centos>`
-  `CentOS Minimal </user/templates/minimal-templates>`
-  :doc:`Gentoo </external/os-guides/gentoo>`
-  `Gentoo Minimal </user/templates/minimal-templates>`
...
-  When a template version you’re using reaches
   `end-of-life </user/how-to-guides/how-to-update.html#upgrading-to-avoid-eol>`__.
...
`here </introduction/faq.html#when-i-try-to-install-a-template-it-says-no-match-is-found>`__.

If you wish to install a template that is in testing, please see
`here </user/downloading-installing-upgrading/testing.html#templates>`__.
...
the template is default. In other cases, please see :doc:`VM
Troubleshooting </user/troubleshooting/vm-troubleshooting>`.
...
4. Base the `disposable template </user/reference/glossary.html#disposable-template>`__
   on the new template.
...
| 3Includes `disposable
  templates </user/reference/glossary.html#disposable-template>`__
...
-  Only install packages from trusted sources – e.g. from the
   ...
   `firewall qube level </user/security-in-qubes/firewall>`), by not allowing any
   networking connectivity in the default template, except for access to
   the Fedora repos.

-  Use `standalones </user/reference/glossary.html#standalone>`__ (see below) for
...
have networking restrictions enforced by the :doc:`firewall
qube </user/security-in-qubes/firewall>`, and again they should not fear this proverbial
...
-  ``qvm-trim-template`` is no longer necessary or available in Qubes
   ...
   handled automatically. No user action is required. See :doc:`Disk
   Trim </external/configuration-guides/disk-trim>` for more information.
...
-  Some templates are available in ready-to-use binary form, but some of
   ...
   details of the template system, please see :doc:`Template
   Implementation </developer/system/template-implementation>`. Take a look at the
