...
written as ``<major>.<minor>.<patch>``. When ``<patch>`` is zero, it is
often omitted as a matter of convention, e.g., ``4.1`` is short for
``4.1.0``.
...
for `testing </user/downloading-installing-upgrading/testing>`. All releases are versions, but not all
...
branch. Please see :doc:`help, support, mailing lists, and
forum </introduction/support>` for places to ask questions about stable releases.
...
release `milestones </introduction/issue-tracking.html#milestones>`__.
...
the Committee takes bug `priorities </introduction/issue-tracking.html#priority>`__
