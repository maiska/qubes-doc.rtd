...
-  ``qubes.Backup`` - store Qubes backup. The service receives location
   ...
   archive (:doc:`description of backup
   format </user/how-to-guides/backup-emergency-restore-v2>`)
...
-  ``qubes.Filecopy`` - receive some files from other VM. Files sent in
   `qfile format </developer/services/qfilecopy>`
-  ``qubes.OpenInVM`` - open a file in called VM. Service receives a
   single file on stdin (in `qfile format </developer/services/qfilecopy>`. After a
...
-  ``qubes.Restore`` - retrieve Qubes backup. The service receives
   ...
   then should output backup archive in :doc:`qfile
   format </developer/services/qfilecopy>` (core-agent-linux component contains
...
-  ``policy.Ask``, ``policy.Notify`` - confirmation prompt and
   ...
   implementation </developer/services/qrexec-internals.html#qrexec-policy-implementation>`__
